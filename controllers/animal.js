...
exports.deleteConge = (req, res)=>{
    ...
    Animal.findByIdAndDelete(id)
        .then(data => {
            if(!data){
                res.status(404).send({ message : `Cannot Delete ANIMAL with id ${id}. Maybe id is wrong`})
            }else{
                res.send({
...
exports.updatee = (req, res)=>{
    ...
    Animal.findByIdAndUpdate(id, req.body, { useFindAndModify: false})
        .then(data => {
            if(!data){
                res.status(404).send({ message : `Cannot Update animal with ${id}. Maybe user not found!`})
            }else{
                res.send(data)
            }
        })
